...
namespace Song.Web.manage
{
    public partial class Products : System.Web.UI.Page
    {
        ...
        public String action = "", ClassName = "", ComUrl = "", pid = "", mid = "",MoreNo="", rid = "", bid = "", sid = "", id = "", FieldList = "";
            ...
            if (!IsPostBack)
            {
                switch (action)
                {
                    case "addproduct"://添加信息
                        ...
                        webcom.BindProType(this.producttype, " and pid=" + 192 + " and fid=0 ");//加载信息类别
                        break;
                    case "edit":
                        webcom.BindProType(this.producttype, " and pid=" + 192 + "  and fid=0 ");//加载信息类别
                        loadedit();
                    ...
                    case "down"://往前移一位
                        ...
                        break;
                    default:
                        webcom.BindProType(this.searchtype, "  and pid=192 and fid=0  ");//加载信息类别
                        bindData();
                        break;
                }
            }
        ...
        {
            String filepath = "", filepathbig = "", sql = "", _type1 = "0", _moreno = "", _type2 = "0", _type3 = "0", _type4 = "0";
            ...
            modelProducts.brand = "";
            modelProducts.protype = Convert.ToInt32(_type1);
            ...
            modelProducts.text2 = this.text2.Text;
            modelProducts.text3 = "";
            modelProducts.text4 = "";
            modelProducts.orderid = maxid;
            modelProducts.hit = 0;
            modelProducts.moreno = _moreno;

            //判断是添加数据还是修改数据 
            if (action == "addproduct")
            {
                bllProducts.Add(modelProducts);
                MessageBox.ShowAndRedirect(this, "添加信息信息成功！", "products.aspx?" + ComUrl + "&action=manage");
            }
            else
            {
                ...
                MessageBox.ShowAndRedirect(this, "修改信息信息成功！", "products.aspx?" + ComUrl + "&action=manage");
            }
        ...
        protected void DeleteInfo()
        {
            if (id != null && id != "")
            {
                ...
                MessageBox.ShowAndRedirect(this, "删除数据成功!", "products.aspx?" + ComUrl + "&action=manage");
            }
            else
            {
                MessageBox.ShowAndRedirect(this, "请选择要删除的信息！", "products.aspx?action=manage&" + ComUrl);
            }
        }
        ...
        protected void bindData()
        {
            ...
            {
                sqlWhere.Append(" and protype=" + bid);
            ...
            if (pageIndex == 0)
            {
                sqlStr.Append("select top " + pageSize.ToString() + " id,title,photo,entitle,prono,saleprice,price,protype,type2,timeinfo,isRecom,isShow,orderid,text2,content from products where 1=1  ");
            }
            else
            {
                sqlStr.Append("select top " + pageSize.ToString() + " id,title,photo,entitle,prono,saleprice,price,protype,type2,timeinfo,isRecom,isShow,orderid,text2,content from products where id not in( select top " + Convert.ToString(pageIndex * pageSize) + " id from products where 1=1 " + Convert.ToString(sqlWhere) + "  order by orderid desc,id desc )   ");
            }
        ...
        protected void loadedit()
        {
            ...
            if (id != null && id.Length != 0)
            {
                ...
                if (modelProducts != null)
                {
                    ...
                    String _moreno = modelProducts.moreno;

                    if (!string.IsNullOrEmpty(_moreno))
                    {
                        MoreNo = _moreno;
                    }
                    else
                    ...
                    this.text2.Text = modelProducts.text2;
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Redirects("Products.aspx?" + ComUrl + "&bid=" + searchtype + "&sid=" + searchtype2 + "&key=" + searchkey);
        }

        protected void delectmessage_Click1(object sender, EventArgs e)
        {
            MessageBox.Redirects("products.aspx?" + ComUrl + "&action=addproduct");
        }
        ...
        protected void manageproductlist_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "Del":
                    ...
                    break;
                case "Update":
                    Response.Redirect("Products.aspx?" + ComUrl + "&action=edit&id=" + id);
                    break;
            }
            bindData();
        }
